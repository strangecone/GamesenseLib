...
do -- Library Functions
    ...
    function Library:ClampString(String, MaxWidth)
        ...
        local TextLabel = Library:CreateObject("TextLabel", {
            ...
            Parent = Client and Client:FindFirstChild("PlayerGui") or CoreGui
        })
    ...
    function Library:CreateObject(Type, Properties, Hidden)
        local HiddenValue = Hidden or false
        ...
        for Index, Value in pairs(Properties) do
            ...
            if Index == "TextStrokeTransparency" and Value == 0 then
                ...
                Library.Objects[Stroke] = {Stroke, {Parent = Object, LineJoinMode = Enum.LineJoinMode.Miter}, HiddenValue}
            else
                Object[Index] = Value
            end
        end
        Library.Objects[Object] = {Object, Properties, HiddenValue}
        return Object
    end
    --
    ...
    function Library:UpdateConfigList(List, Type)
        for _, File in pairs(listfiles("gamesense/Configs")) do
    ...
    function Library:GetObjectsTable(MainUI, AddMain, Ignored)
        ...
        for _, Descendant in pairs(MainUI:GetDescendants()) do
            ...
            table.insert(DescendantTable, Descendant)
        end
        if AddMain then
            table.insert(DescendantTable, MainUI)
        end
        for _, Descendant in pairs(DescendantTable) do
            ...
            if Found then
                ...
                table.insert(NewTable, {Descendant, Properties, HiddenValue})
            end
        end
        return NewTable
    end
    ...
    function Library:ViewPlayer(Player)
        if not Library.UI.Viewing then
            ...
            if Camera and Client.Character and Client.Character:FindFirstChild("Humanoid") then
    ...
    function Library:ColorPicker(Options)
        ...
        do -- Functions
            ...
            function ColorPicker:RemoveFrame(Fast)
                ...
                for Index, Value in pairs(Library.UI.ScreenGUI:GetChildren()) do
                    if Value:IsA("Frame") and Value.Name == "ColorPickerFrame" .. Library.UI.TotalColorPickers then
                        if Fast then Value:Destroy() else Library:Fade(false, Library:GetObjectsTable(Value, true), Value, 0.1) task.delay(Library.UI.TweenSpeed, function() Value:Destroy() end) end
                    end
                end
            end
            ...
            function ColorPicker:Toggle()
                ...
                if not ColorPicker:FindFrame() then ColorPicker.Active = true ColorPicker:AddFrame() else ColorPicker.Active = false ColorPicker:RemoveFrame() end
            end
            ...
            function ColorPicker:AddOtherFrame()
                ...
                for Index, Value in pairs({"Copy", "Paste", "Reset"}) do
                    ...
                    Library:Connection(Inactive.MouseEnter, function() Inactive.BackgroundColor3 = Color3.fromRGB(25, 25, 25) if ModeItem.Active then return end Inactive.Text = "<b>" .. Value .. "</b>" end)
            ...
            function ColorPicker:ToggleOtherFrame()
                ...
                if not ColorPicker:FindOtherFrame() then ColorPicker.ActiveFrame = true ColorPicker:AddOtherFrame() else ColorPicker.ActiveFrame = false ColorPicker:RemoveOtherFrame() end
            end
        end
    ...
    function Library:Keybind(Options)
        ...
        local Keybind = {
            Hover = false, ActiveFrame = false, Keybind = Options.Default,
            RegKeybind = nil, State = false, SelectingKeybind = false,
            Toggle = false, Connection = nil, Mode = Options.Mode,
            ConfigKeybind = nil, Current = {}, CurrentMode = nil, Hiding = false,
        }
        ...
        do -- Functions
            function Keybind:SetVisible(Bool)
                ...
                Library:TweenObject(KeybindObject, TweenInfo.new(Library.UI.TweenSpeed, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {Size = Bool and UDim2.new(1, 0, 0, 8) or UDim2.new(1, 0, 0, -10)}, function() if not Bool then Library.Objects[KeybindObject] = {KeybindObject, OldValues[2], false} end end)
            end
            ...
            function Keybind:Get() return Keybind.RegKeybind.EnumType == Enum.KeyCode and tostring(Keybind.RegKeybind):match("^Enum%.KeyCode%.(.+)$") or tostring(Keybind.RegKeybind):match("^Enum%.UserInputType%.(.+)$") end
            ...
            function Keybind:ToggleFrame()
                ...
                if not Keybind:FindFrame() then Keybind.ActiveFrame = true Keybind:AddFrame() else Keybind.ActiveFrame = false Keybind:RemoveFrame() end
            end
        end
        ...
        Library:Connection(Button_4.MouseButton1Click, function()
            ...
            Keybind.Connection = Library:Connection(UserInputService.InputBegan, function(Input)
                ...
                if Keybind.Connection then Keybind.Connection:Disconnect() task.delay(0.1, function() Keybind.Connection = nil Keybind.SelectingKeybind = false end) end
            end)
        end)
    ...
    function Library:MultiBox(Options)
        ...
        local MultiBox = {
            Open = false, Hover = false, Items = Options.Content, Scrollable = false,
            Value = {}, SelectedOrder = {}, AllItems = {},
        }
        ...
        do -- Functions
            ...
            function MultiBox:AddValue(Value)
                ...
                function Item:GetSelectedItems()
                    local SelectedItems = {}
                    for _, i in pairs(MultiBox.SelectedOrder) do
                        if MultiBox.Items[i] then table.insert(SelectedItems, i) end
                    end
                ...
                function Item:SelectItem(itm)
                    if not table.find(MultiBox.SelectedOrder, itm) then table.insert(MultiBox.SelectedOrder, itm) end
                    MultiBox:UpdateValue()
                end
                function Item:DeselectItem(itm)
                    for k, v in pairs(MultiBox.SelectedOrder) do
                        if v == itm then table.remove(MultiBox.SelectedOrder, k) break end
                    end
                    MultiBox:UpdateValue()
                end
    ...
    function Library:Dropdown(Options)
        ...
        local Dropdown = {
            Open = false, Hovering = false, CurrentItem = nil, Scrollable = false,
            Hiding = false, Items = {}, Value = Options.Default,
        }
        ...
        do -- Functions
            ...
            function Dropdown:Refresh(NewOptions, DeleteCurrent)
                ...
                for _, val in pairs(NewOptions) do Dropdown:AddValue(val) end
            end
        end
    ...
    function Library:Toggle(Options)
        ...
        local ToggleMain = Library:CreateObject("Frame", {Size = UDim2.new(1, -2, 1, -2), Name = "ToggleInline", Position = UDim2.new(0, 1, 0, 1), BorderColor3 = Color3.fromRGB(0, 0, 0), ZIndex = Options.ZIndex or 2, BorderSizePixel = 0, BackgroundTransparency = 1, BackgroundColor3 = Color3.fromRGB(255, 255, 255), Parent = ToggleOutline})
        ...
        do -- Functions
            ...
            function Toggle:SetVisible(Bool)
                ...
                Library:TweenObject(PreviewToggle, TweenInfo.new(Library.UI.TweenSpeed, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {Size = Bool and UDim2.new(1, 0, 0, 8) or UDim2.new(1, 0, 0, -10)}, function() if not Bool then Library.Objects[PreviewToggle] = {PreviewToggle, OldValues[2], false} end end)
            end
            
            function Toggle:ColorPicker(Options)
                Options = Library:Validate({Name = "Preview Color Picker", Default = Library.Theme.Default.Accent, Flag = Library.NewFlag(), Alpha = 0, AlphaBar = true, Callback = function() end}, Options or {})
                local ColorPicker = {} Toggle.ColorPickers[#Toggle.ColorPickers + 1] = ColorPicker
                local ColorPickerFrame = Library:ColorPicker({Name = Options.Name, Default = Options.Default, Flag = Options.Flag, Alpha = Options.Alpha, AlphaBar = Options.AlphaBar, MainUI = Toggle.MainUI, TabUI = Toggle.TabUI, Callback = Options.Callback, Parent = PreviewToggle, Keybind = Toggle.KeybindState, Count = #Toggle.ColorPickers})
                return ColorPickerFrame
            end
            
            function Toggle:Keybind(Options)
                Options = Library:Validate({Default = Enum.KeyCode.Backspace, Mode = "Toggle", UseMode = true, HideFromList = false, Blacklisted = {}, Hiding = false, ChangeToggle = false, Flag = Library.NewFlag(), Callback = function() end}, Options or {})
                local Keybind = {} Toggle.KeybindState = true
                Library:Keybind({Default = Options.Default, Mode = Options.Mode, HideFromList = Options.HideFromList, Blacklisted = Options.Blacklisted, Parent = PreviewToggle, UseMode = Options.UseMode, Toggle = Toggle, MainUI = Toggle.MainUI, TabUI = Toggle.TabUI, Hiding = Options.Hiding, ToggleState = Toggle.State, ChangeToggle = Options.ChangeToggle, Flag = Options.Flag, Callback = Options.Callback, Count = #Toggle.ColorPickers + 1})
                return Keybind
            end
        end
    ...
    function Library:Label(Options)
        ...
        do -- Functions
            ...
            function Label:ColorPicker(Options)
                Options = Library:Validate({Name = "Preview Color Picker", Default = Library.Theme.Default.Accent, Flag = Library.NewFlag(), Alpha = 0, AlphaBar = true, MainUI = nil, Callback = function() end}, Options or {})
                local ColorPicker = {} Label.ColorPickers[#Label.ColorPickers + 1] = ColorPicker
                local ColorPickerFrame = Library:ColorPicker({Name = Options.Name, Default = Options.Default, Flag = Options.Flag, Alpha = Options.Alpha, AlphaBar = Options.AlphaBar, MainUI = Label.MainUI, TabUI = Label.TabUI, Callback = Options.Callback, Parent = PreviewLabel, Keybind = Label.KeybindState, Count = #Label.ColorPickers})
                return ColorPickerFrame
            end
            
            function Label:Keybind(Options)
                Options = Library:Validate({Default = Enum.KeyCode.Backspace, Mode = "Toggle", UseMode = true, HideFromList = false, Blacklisted = {}, Hiding = false, Flag = Library.NewFlag(), Callback = function() end}, Options or {})
                local Keybind = {} Label.KeybindState = true
                Library:Keybind({Default = Options.Default, Mode = Options.Mode, HideFromList = Options.HideFromList, Blacklisted = Options.Blacklisted, Parent = PreviewLabel, Toggle = Label, UseMode = Options.UseMode, MainUI = Label.MainUI, TabUI = Label.TabUI, Hiding = Options.Hiding, ToggleState = Label.State, Flag = Options.Flag, Callback = Options.Callback, Count = #Label.ColorPickers + 1})
                return Keybind
            end
        end
    ...
    function Library:TextBox(Options)
        ...
        do -- Connections
            ...
            Library:Connection(TextBoxObject.Focused, function()
                ...
                TextBox.Focused = true TextBoxObject.TextColor3 = Library.Theme.Default.Accent Library:AddTheme(TextBoxObject, {TextColor3 = "Accent"}) TextBoxObject.PlaceholderText = ""
            end)
            Library:Connection(TextBoxObject.FocusLost, function(EnterPressed)
                ...
                TextBox.Focused = false TextBoxObject.PlaceholderText = "_" TextBoxObject.TextColor3 = Library.Theme.Default.TextColor Library:AddTheme(TextBoxObject, {TextColor3 = "TextColor"})
                Library.Flags[Options.Flag] = TextBox Options.Callback(TextBoxObject.Text)
            end)
        end
    ...
    function Library:Button(Options)
        ...
        do -- Functions
            ...
            function Button:SetVisible(Bool)
                ...
                Library:TweenObject(PreviewButton, TweenInfo.new(Library.UI.TweenSpeed, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {Size = Bool and Options.Size or UDim2.new(1, 0, 0, -10)}, function() if not Bool then Library.Objects[PreviewButton] = {PreviewButton, OldValues[2], false} end end)
            end
            function Button:ConfirmationStart()
                Button.MouseDown = true Button.WaitingForConfirm = true Button.ConfirmationTime = 3
                ...
                Button.ConfirmationConnection = coroutine.create(function()
                    for i = 1, 3 do 
                        ...
                        if Button.ConfirmationTime <= 0 then
                            ...
                            if Button.MouseDown then Library:TweenObject(Button_6, TweenInfo.new(Library.UI.TweenSpeed, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {TextColor3 = Library.Theme.Default.TextColor}) Button.MouseDown = false Button.WaitingForConfirm = false end
                            break
                        end
                    end
                end)
        ...
        do -- Connections
            Library:Connection(Button_6.MouseButton1Down, function()
                ...
                if Options.Confirmation then
                    ...
                    Options.Callback() Button.MouseDown = true
                end
            end)
            Library:Connection(UserInputService.InputEnded, function(Input)
                if Input.UserInputType == Enum.UserInputType.MouseButton1 and Button.MouseDown and not Button.WaitingForConfirm then Button.Hovering = false Button.MouseDown = false end
