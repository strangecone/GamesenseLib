--[[
    GamesenseLib – A drop‑in replacement for Orion Library
    Based on the gamesense UI library (internal code unchanged).
    Exposes the exact same API as Orion.
]]
...
-- ============================================================
-- === GamesenseLib – Orion‑style wrapper                     ===
-- ============================================================
...
local function getDefaultSection(tab)
    if not GamesenseLib._tabDefaultSections[tab] then
        -- Create a default section with a generic name (empty)
...
function GamesenseLib:MakeWindow(config)
    ...
    local window = Library:Window({
        Name = name,
        CloseBind = Enum.KeyCode.Insert, -- can be changed later
    })
    self._window = window
    ...
    return {
        MakeTab = function(_, tabConfig)
            ...
            -- Store the tab for default section
            ...
            return {
                AddSection = function(_, sectionConfig)
                    ...
                    -- Return section with element methods
                    return {
                        ...
                        AddToggle = function(_, togConfig)
                            ...
                            -- Add Save support: if Save is true, we store the flag for later
                            if togConfig.Save then
                                -- Gamesense already stores in Library.Flags via Flag
                            end
                            return toggle
                        end,
                        AddSlider = function(_, sliConfig)
                            ...
                            return section:Slider({
                                ...
                                -- Color not supported directly, but we can set accent via UpdateColor later
                            })
                        end,
                        ...
                        AddColorpicker = function(_, cpConfig)
                            ...
                            -- Orion's colorpicker is standalone; we'll add a label with a colorpicker attached
                        ...
                        AddParagraph = function(_, paraConfig)
                            -- Gamesense has no paragraph element; we'll use a label with multi-line text
                        ...
                        -- For other elements not directly supported, we can add stub functions
                ...
                -- Direct element methods (uses default section)
                AddButton = function(_, btnConfig)
                    local sec = getDefaultSection(tab)
                    return sec:Button({
                        Name = btnConfig.Name or "Button",
                        Callback = btnConfig.Callback or function() end,
                        Confirmation = btnConfig.Confirmation or false,
                ...
                AddColorpicker = function(_, cpConfig)
                    local sec = getDefaultSection(tab)
                    local label = sec:Label({ Message = cpConfig.Name or "Colorpicker" })
                ...
                AddBind = function(_, bindConfig)
                    local sec = getDefaultSection(tab)
                    local label = sec:Label({ Message = bindConfig.Name or "Bind" })
                ...
                AddLabel = function(_, labelConfig)
                    local sec = getDefaultSection(tab)
                    return sec:Label({ Message = labelConfig.Text or "Label" })
                end,
                AddParagraph = function(_, paraConfig)
                    local sec = getDefaultSection(tab)
                    local text = paraConfig.Text or "Text"
                    local content = paraConfig.Content or "Content"
                    return sec:Label({ Message = text .. "\n" .. content })
                end,
                AddTextbox = function(_, tbConfig)
                    local sec = getDefaultSection(tab)
                    return sec:TextBox({
