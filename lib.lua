-- True Gamesense (Skeet) UI Library
-- 1:1 Orion API Parity, Perfect Aesthetics, Smart Trackers, Working Configs.
...
local function CreateDoubleBorder(parent, size, pos, bgColor)
	...
	Outer.Size = size Outer.Position = pos Outer.BackgroundColor3 = GameSenseLib.Theme.Outline1 Outer.BorderSizePixel = 0
	local Inner = Instance.new("Frame", Outer)
	Inner.Size = UDim2.new(1, -2, 1, -2) Inner.Position = UDim2.new(0, 1, 0, 1) Inner.BackgroundColor3 = GameSenseLib.Theme.Outline2 Inner.BorderSizePixel = 0
	local Main = Instance.new("Frame", Inner)
	Main.Size = UDim2.new(1, -2, 1, -2) Main.Position = UDim2.new(0, 1, 0, 1) Main.BackgroundColor3 = bgColor or GameSenseLib.Theme.Background Main.BorderSizePixel = 0
	return Outer, Main
end
...
function GameSenseLib:MakeWindow(options)
	...
	self.WatermarkText = options.WatermarkText or "gamesense [beta]"
	...
	-- Main Window (Taller shape)
	local MainOuter, MainInnerBg = CreateDoubleBorder(ScreenGui, UDim2.new(0, 660, 0, 600), UDim2.new(0.5, -330, 0.5, -300), self.Theme.Background)
	MainOuter.Visible = true
	MakeDraggable(MainOuter, MainOuter, function() return true end)

	-- POPUPS (Attached to ScreenGui to fix absolute position offsets)
	...
	function GameSenseLib:MakeNotification(opt)
		...
		local Outer, MainBg = CreateDoubleBorder(NotifContainer, UDim2.new(1, 0, 0, 50), UDim2.new(1, 300, 0, 0), self.Theme.Background)
		Outer.BackgroundTransparency = 1 MainBg.BackgroundTransparency = 1 MainBg.Parent.BackgroundTransparency = 1
		
		local NGrad = Instance.new("Frame", MainBg)
		NGrad.Size = UDim2.new(1, 0, 0, 2)
		...
		local NTitle = Instance.new("TextLabel", MainBg)
		NTitle.Size = UDim2.new(1, -10, 0, 15) NTitle.Position = UDim2.new(0, 5, 0, 5) NTitle.BackgroundTransparency = 1
		NTitle.Text = title NTitle.TextColor3 = self.Theme.Text NTitle.Font = self.Theme.Font NTitle.TextSize = 13 NTitle.TextXAlignment = Enum.TextXAlignment.Left NTitle.TextTransparency = 1

		local NContent = Instance.new("TextLabel", MainBg)
		NContent.Size = UDim2.new(1, -10, 1, -25) NContent.Position = UDim2.new(0, 5, 0, 20) NContent.BackgroundTransparency = 1
		NContent.Text = content NContent.TextColor3 = self.Theme.DarkText NContent.Font = self.Theme.Font NContent.TextSize = 12 
		NContent.TextXAlignment = Enum.TextXAlignment.Left NContent.TextYAlignment = Enum.TextYAlignment.Top NContent.TextWrapped = true NContent.TextTransparency = 1
		
		local TI = TweenInfo.new(0.4, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out)
		TweenService:Create(Outer, TI, {Position = UDim2.new(0, 0, 0, 0), BackgroundTransparency = 0}):Play()
		TweenService:Create(MainBg, TI, {BackgroundTransparency = 0}):Play() TweenService:Create(MainBg.Parent, TI, {BackgroundTransparency = 0}):Play()
		TweenService:Create(NGrad, TI, {BackgroundTransparency = 0}):Play() TweenService:Create(NTitle, TI, {TextTransparency = 0}):Play() TweenService:Create(NContent, TI, {TextTransparency = 0}):Play()
		
		task.delay(time, function()
			local TO = TweenInfo.new(0.4, Enum.EasingStyle.Exponential, Enum.EasingDirection.In)
			TweenService:Create(Outer, TO, {Position = UDim2.new(1, 300, 0, 0), BackgroundTransparency = 1}):Play()
			TweenService:Create(MainBg, TO, {BackgroundTransparency = 1}):Play() TweenService:Create(MainBg.Parent, TO, {BackgroundTransparency = 1}):Play()
			TweenService:Create(NGrad, TO, {BackgroundTransparency = 1}):Play() TweenService:Create(NTitle, TO, {TextTransparency = 1}):Play()
			local out = TweenService:Create(NContent, TO, {TextTransparency = 1}) out:Play() out.Completed:Connect(function() Outer:Destroy() end)
		end)
	end

	-- SMART WATERMARK (Anchored to prevent going off-screen)
	...
	MakeDraggable(WMOuter, WMOuter, function() return MainOuter.Visible end) -- Draggable ONLY when menu open
	...
	-- KEYBINDS LIST (Matches image_e633c7.png perfectly)
	...
	MakeDraggable(KBOuter, KBOuter, function() return MainOuter.Visible end) -- Draggable ONLY when menu open
	
	local KBGrad = WMGradient:Clone()
	KBGrad.Parent = KBInner KBGrad.Size = UDim2.new(1, 0, 0, 2)
	...
	getgenv().GameSense_Input = UserInputService.InputBegan:Connect(function(input, gpe)
		...
		if input.KeyCode == GameSenseLib.MenuKey or input.KeyCode == Enum.KeyCode.RightShift then
	...
	function WindowObj:MakeTab(tabOptions)
		...
		IconImg.ImageColor3 = GameSenseLib.Theme.DarkText -- Dim
		...
		local LeftLayout = Instance.new("UIListLayout", LeftCol) LeftLayout.Padding = UDim.new(0, 15)
		local LPad = Instance.new("UIPadding", LeftCol) LPad.PaddingTop = UDim.new(0, 10) LPad.PaddingBottom = UDim.new(0, 10)

		local RightCol = Instance.new("ScrollingFrame", TabContent)
		RightCol.Size = UDim2.new(0.5, -5, 1, 0) RightCol.Position = UDim2.new(0.5, 5, 0, 0) RightCol.BackgroundTransparency = 1 RightCol.ScrollBarThickness = 0
		local RightLayout = Instance.new("UIListLayout", RightCol) RightLayout.Padding = UDim.new(0, 15)
		local RPad = Instance.new("UIPadding", RightCol) RPad.PaddingTop = UDim.new(0, 10) RPad.PaddingBottom = UDim.new(0, 10)
		...
		TabBtn.MouseButton1Click:Connect(function()
			...
			IconImg.ImageColor3 = Color3.new(1,1,1) -- Bright active tab
			ClosePopups()
		end)
		...
		function TabObj:AddSection(secOptions)
			...
			function SectionObj:AddToggle(opt)
				...
				Frame.Size = UDim2.new(1, 0, 0, 12) Frame.BackgroundTransparency = 1 Frame.Text = ""
				...
				Label.Size = UDim2.new(1, -20, 1, 0) Label.Position = UDim2.new(0, 18, 0, 0) Label.BackgroundTransparency = 1
				Label.Text = opt.Name or "Toggle" Label.TextColor3 = GameSenseLib.Theme.Text Label.Font = GameSenseLib.Theme.Font Label.TextSize = 12 Label.TextXAlignment = Enum.TextXAlignment.Left
			...
			function SectionObj:AddSlider(opt)
				...
				Label.Text = opt.Name or "Slider" Label.TextColor3 = GameSenseLib.Theme.Text Label.Font = GameSenseLib.Theme.Font Label.TextSize = 12 Label.TextXAlignment = Enum.TextXAlignment.Left
				...
				Fill.Size = UDim2.new((val - opt.Min) / (opt.Max - opt.Min), 0, 1, 0) Fill.BackgroundColor3 = GameSenseLib.Theme.Accent Fill.BorderSizePixel = 0
				GameSenseLib.AccentObjects[Fill] = "BackgroundColor3"

				local ValLabel = Instance.new("TextLabel", TrackOut)
				ValLabel.Size = UDim2.new(1, -2, 1, 0) ValLabel.BackgroundTransparency = 1
				ValLabel.Text = tostring(val) .. (opt.ValueName or "") ValLabel.TextColor3 = Color3.new(1,1,1) ValLabel.Font = GameSenseLib.Theme.Font ValLabel.TextSize = 10 ValLabel.TextXAlignment = Enum.TextXAlignment.Right ValLabel.ZIndex = 5
			...
			function SectionObj:AddDropdown(opt)
				...
				local Label = Instance.new("TextLabel", Frame)
				Label.Size = UDim2.new(1, 0, 0, 15) Label.BackgroundTransparency = 1
				Label.Text = opt.Name or "Dropdown" Label.TextColor3 = GameSenseLib.Theme.Text Label.Font = GameSenseLib.Theme.Font Label.TextSize = 12 Label.TextXAlignment = Enum.TextXAlignment.Left
				...
				MainBtn.Text = " " .. tostring(selected) MainBtn.TextColor3 = GameSenseLib.Theme.Text MainBtn.Font = GameSenseLib.Theme.Font MainBtn.TextSize = 12 MainBtn.TextXAlignment = Enum.TextXAlignment.Left
				...
				MainBtn.MouseButton1Click:Connect(function()
					...
					local DropOut, DropIn = CreateDoubleBorder(PopupContainer, UDim2.new(0, MainOut.AbsoluteSize.X, 0, #(opt.Options or {}) * 16), UDim2.new(0, MainOut.AbsolutePosition.X, 0, MainOut.AbsolutePosition.Y + 18), GameSenseLib.Theme.Outline2)
					Instance.new("UIListLayout", DropIn)

					for _, item in ipairs(opt.Options or {}) do
						local Btn = Instance.new("TextButton", DropIn)
						Btn.Size = UDim2.new(1, 0, 0, 16) Btn.BackgroundColor3 = GameSenseLib.Theme.Outline2 Btn.BorderSizePixel = 0
						Btn.Text = " " .. tostring(item) Btn.TextColor3 = GameSenseLib.Theme.Text Btn.Font = GameSenseLib.Theme.Font Btn.TextSize = 12 Btn.TextXAlignment = Enum.TextXAlignment.Left
			...
			function SectionObj:AddBind(opt)
				...
				local Frame = Instance.new("Frame", ItemContainer)
				Frame.Size = UDim2.new(1, 0, 0, 15) Frame.BackgroundTransparency = 1

				local Label = Instance.new("TextLabel", Frame)
				Label.Size = UDim2.new(1, -50, 1, 0) Label.BackgroundTransparency = 1
				Label.Text = opt.Name or "Bind" Label.TextColor3 = GameSenseLib.Theme.Text Label.Font = GameSenseLib.Theme.Font Label.TextSize = 12 Label.TextXAlignment = Enum.TextXAlignment.Left
				...
				Btn.Text = "[" .. (key == Enum.KeyCode.Unknown and "None" or key.Name) .. "]" Btn.TextColor3 = GameSenseLib.Theme.DarkText Btn.Font = GameSenseLib.Theme.Font Btn.TextSize = 11
				
				local KBFrame = Instance.new("Frame", KBContainer)
				KBFrame.Size = UDim2.new(1, 0, 0, 16) KBFrame.BackgroundTransparency = 1 KBFrame.Visible = false
				local KBName = Instance.new("TextLabel", KBFrame)
				KBName.Size = UDim2.new(0.5, 0, 1, 0) KBName.Position = UDim2.new(0, 5, 0, 0) KBName.BackgroundTransparency = 1 KBName.TextColor3 = GameSenseLib.Theme.Text KBName.Font = GameSenseLib.Theme.Font KBName.TextSize = 12 KBName.TextXAlignment = Enum.TextXAlignment.Left
				local KBState = Instance.new("TextLabel", KBFrame)
				KBState.Size = UDim2.new(0.5, -5, 1, 0) KBState.Position = UDim2.new(0.5, 0, 0, 0) KBState.BackgroundTransparency = 1 KBState.TextColor3 = GameSenseLib.Theme.Text KBState.Font = GameSenseLib.Theme.Font KBState.TextSize = 12 KBState.TextXAlignment = Enum.TextXAlignment.Right
				...
				local function Set(newKey)
					...
					if opt.OnKeyChange then pcall(opt.OnKeyChange, newKey) end
					if opt.Save and GameSenseLib.AutoSaveEnabled then GameSenseLib:SaveConfig("auto_config", true) end
				end
				...
				Btn.MouseButton2Click:Connect(function()
					...
					local DropOut, DropIn = CreateDoubleBorder(PopupContainer, UDim2.new(0, 60, 0, #Modes * 16), UDim2.new(0, BtnOut.AbsolutePosition.X, 0, BtnOut.AbsolutePosition.Y + 18), GameSenseLib.Theme.Outline2)
					Instance.new("UIListLayout", DropIn)

					for _, m in ipairs(Modes) do
						local MBtn = Instance.new("TextButton", DropIn)
						MBtn.Size = UDim2.new(1, 0, 0, 16) MBtn.BackgroundColor3 = GameSenseLib.Theme.Outline2 MBtn.BorderSizePixel = 0
						...
						MBtn.Font = GameSenseLib.Theme.Font MBtn.TextSize = 11 MBtn.TextXAlignment = Enum.TextXAlignment.Left
				...
				UserInputService.InputBegan:Connect(function(input, gpe)
					if listening and input.UserInputType == Enum.UserInputType.Keyboard then
						Set(input.KeyCode) listening = false
			...
			function SectionObj:AddColorpicker(opt)
				...
				local Label = Instance.new("TextLabel", Frame)
				Label.Size = UDim2.new(1, -30, 1, 0) Label.BackgroundTransparency = 1
				Label.Text = opt.Name or "Color" Label.TextColor3 = GameSenseLib.Theme.Text Label.Font = GameSenseLib.Theme.Font Label.TextSize = 12 Label.TextXAlignment = Enum.TextXAlignment.Left
				...
				Btn.MouseButton1Click:Connect(function()
					...
					local PickOut, PickIn = CreateDoubleBorder(PopupContainer, UDim2.new(0, 150, 0, 170), UDim2.new(0, BtnOut.AbsolutePosition.X - 125, 0, BtnOut.AbsolutePosition.Y + 15), GameSenseLib.Theme.GroupboxBg)
					...
					SVMap.Size = UDim2.new(1, 0, 1, 0) SVMap.BackgroundTransparency = 1 SVMap.Text = ""
					activeSVMap = SVIn
					...
					HueMap.Size = UDim2.new(1, 0, 1, 0) HueMap.BackgroundTransparency = 1 HueMap.Text = ""
					...
					local RbOut, RbIn = CreateDoubleBorder(PickIn, UDim2.new(0, 130, 0, 14), UDim2.new(0, 8, 0, 148), GameSenseLib.Theme.GroupboxBg)
					local RbBtn = Instance.new("TextButton", RbIn)
					RbBtn.Size = UDim2.new(1, 0, 1, 0) RbBtn.BackgroundTransparency = 1
					...
					RbBtn.Font = GameSenseLib.Theme.Font RbBtn.TextSize = 11
			...
			function SectionObj:AddTextbox(opt)
				...
				local Label = Instance.new("TextLabel", Frame)
				Label.Size = UDim2.new(1, 0, 0, 15) Label.BackgroundTransparency = 1
				Label.Text = opt.Name or "Textbox" Label.TextColor3 = GameSenseLib.Theme.Text Label.Font = GameSenseLib.Theme.Font Label.TextSize = 12 Label.TextXAlignment = Enum.TextXAlignment.Left
				...
				Box.Text = val Box.TextColor3 = GameSenseLib.Theme.Text Box.Font = GameSenseLib.Theme.Font Box.TextSize = 12 Box.TextXAlignment = Enum.TextXAlignment.Left Box.ClearTextOnFocus = false
			...
			function SectionObj:AddButton(opt)
				...
				local Btn = Instance.new("TextButton", BtnIn)
				Btn.Size = UDim2.new(1, 0, 1, 0) Btn.BackgroundTransparency = 1
				Btn.Text = opt.Name or "Button" Btn.TextColor3 = GameSenseLib.Theme.Text Btn.Font = GameSenseLib.Theme.Font Btn.TextSize = 12
			...
			function SectionObj:AddListbox(opt)
				...
				Scroll.Size = UDim2.new(1, 0, 1, 0) Scroll.BackgroundTransparency = 1 Scroll.ScrollBarThickness = 2 Scroll.BorderSizePixel = 0
				...
				local function Refresh(newList)
					...
					for _, v in ipairs(newList) do
						...
						Btn.Size = UDim2.new(1, 0, 0, 16) Btn.BackgroundTransparency = 1
						Btn.Text = "  " .. tostring(v) Btn.TextColor3 = GameSenseLib.Theme.Text Btn.TextXAlignment = Enum.TextXAlignment.Left Btn.Font = GameSenseLib.Theme.Font Btn.TextSize = 12

						Btn.MouseButton1Click:Connect(function()
							selected = v
							for _, b in pairs(buttons) do b.TextColor3 = GameSenseLib.Theme.Text GameSenseLib.AccentObjects[b] = nil end
							Btn.TextColor3 = GameSenseLib.Theme.Accent GameSenseLib.AccentObjects[Btn] = "TextColor3"
		...
		-- Orion API compatibility (If a user attempts to add an element straight to the Tab, it creates a 'General' groupbox automatically)
	...
	-- PERFECT BUILT-IN SETTINGS & CONFIG TAB
	...
	local function GetConfigs()
		...
		for _, f in ipairs(files) do
			local placeId, name = f:match("(%d+)_([^/\\]+)%.json$")
			if placeId == tostring(game.PlaceId) and name then table.insert(valid, name) end
		end
		return valid
	end

	local CfgBox = ConfigListboxSec:AddListbox({ Options = GetConfigs(), Height = 250, Callback = function(val) SelectedConfig = val end })
	local CfgInput = ConfigListboxSec:AddTextbox({ Name = "Config name", Default = "", TextDisappear = false, Callback = function(v) SelectedConfig = v end })
	...
	ConfigListboxSec:AddButton({ Name = "Delete", Callback = function() 
		if SelectedConfig ~= "" then 
			pcall(function() delfile(GameSenseLib.ConfigFolder .. "/" .. tostring(game.PlaceId) .. "_" .. SelectedConfig .. ".json") end)
			CfgBox:Refresh(GetConfigs())
			GameSenseLib:MakeNotification({Name = "Config Deleted", Content = "Removed " .. SelectedConfig, Time = 3})
		end
	end})
	...
	MainSettings:AddColorpicker({ Name = "Menu Accent Color", Default = GameSenseLib.Theme.Accent, Callback = function(c) GameSenseLib:UpdateAccent(c) end })
